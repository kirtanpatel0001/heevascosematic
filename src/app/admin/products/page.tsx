...
// Default 3 steps for How to Use
const DEFAULT_STEPS = [
      ...
      >
        ...
        {selectedValues.map(val => (
          <span key={val} className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md bg-black text-white text-[11px] font-bold uppercase tracking-wide">
            {val}
            <button onClick={(e) => { e.stopPropagation(); onChange(selectedValues.filter(v => v !== val)); }} className="text-white/70 hover:text-white transition-colors"><X size={11} strokeWidth={3} /></button>
          </span>
        ))}
...
export default function ProductsPage() {
  ...
  const fetchProducts = async () => {
    ...
    const safeData = data?.map((p: any) => ({
        ...
        usage_steps: (p.usage_steps && p.usage_steps.length === 3) ? p.usage_steps : DEFAULT_STEPS,
  ...
  const handleOpenModal = (product?: Product) => {
    if (product) {
        setCurrentProduct({
            ...
            usage_steps: (product.usage_steps && product.usage_steps.length === 3) ? product.usage_steps : DEFAULT_STEPS,
        });
  ...
  const handleSingleUpload = async (e: React.ChangeEvent<HTMLInputElement>, field: 'before_image' | 'after_image') => {
      ...
      setUploading(true);
      const url = await uploadFile(file);
      if (url) setCurrentProduct(prev => ({ ...prev, [field]: url }));
      setUploading(false);
  };

  const updateUsageStep = (index: number, field: keyof UsageStep, value: string) => {
      const newSteps = [...currentProduct.usage_steps];
      newSteps[index] = { ...newSteps[index], [field]: value };
      setCurrentProduct({ ...currentProduct, usage_steps: newSteps });
  }
  ...
  const handleStepImageUpload = async (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setUploading(true);
      const url = await uploadFile(file);
      if(url) updateUsageStep(index, 'image', url);
      setUploading(false);
  }
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FC] p-4 md:p-8 font-sans text-slate-900 overflow-x-hidden">
            ...
            <tbody className="divide-y divide-gray-100 text-sm">
              {products.map(product => (
                <tr key={product.id} className="hover:bg-gray-50 transition-colors group">
                  ...
                  <td className="p-4"><span className="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-bold bg-slate-100 text-slate-700 border border-slate-200">{product.category}</span></td>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className={`text-xs font-bold ${product.stock > 10 ? 'text-green-700' : product.stock > 0 ? 'text-amber-700' : 'text-red-700'}`}>{product.stock > 0 ? `${product.stock} in stock` : 'Out of Stock'}</p>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-white">
              <form id="product-form" onSubmit={handleSave} className="space-y-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  {/* LEFT SECTION (Col 1 & 2 combined logic) */}
                  <div className="lg:col-span-2 space-y-8">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-b border-gray-100 pb-8">
                         ...
                         <div className="space-y-5">
                             ...
                             <div className="mt-4"><label className="input-label">Ingredients</label><textarea rows={3} className="input-field" value={currentProduct.ingredients} onChange={e => setCurrentProduct({...currentProduct, ingredients: e.target.value})} /></div>
                         </div>
                      </div>

                      {/* 4. HOW TO USE STEPS - (Fixed 3 Steps Layout) */}
                      <div>
                         <h3 className="text-xs font-black uppercase text-slate-400 tracking-wider mb-4 flex items-center gap-2">
                            <ListOrdered size={14}/> 4. How to Use (3 Steps)
                         </h3>
                         <div className="grid grid-cols-3 gap-4">
                             {currentProduct.usage_steps.map((step, idx) => (
                                 <div key={idx} className="border border-slate-200 bg-slate-50 p-3 rounded-xl">
                                     <div className="flex flex-col gap-3">
                                         <div className="w-full aspect-[4/3] bg-white border border-slate-200 rounded-lg flex-shrink-0 overflow-hidden relative">
                                             {step.image ? (
                                                 <img src={step.image} className="w-full h-full object-cover"/>
                                             ) : (
                                                 <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer hover:bg-slate-100 text-slate-400 gap-1">
                                                     <input type="file" accept="image/*" className="hidden" onChange={(e) => handleStepImageUpload(idx, e)}/>
                                                     <UploadCloud size={20} />
                                         ...
                                         <div className="flex-1 space-y-2">
                                             <div>
                                                 <span className="text-[9px] font-bold uppercase text-slate-400 block mb-1">Step {idx + 1} Title</span>
                                                 <input className="w-full text-xs font-bold bg-white border border-slate-200 rounded px-2 py-1.5" value={step.title} onChange={(e) => updateUsageStep(idx, 'title', e.target.value)} placeholder="e.g. Apply" />
                                             </div>
                                             <div>
                                                  <span className="text-[9px] font-bold uppercase text-slate-400 block mb-1">Description</span>
                                                 <textarea className="w-full text-xs bg-white border border-slate-200 rounded px-2 py-1.5 resize-none leading-snug" rows={3} value={step.text} onChange={(e) => updateUsageStep(idx, 'text', e.target.value)} placeholder="Instruction..."></textarea>
                                             </div>
                                         </div>
                                     </div>
                                 </div>
                  ...
                  <div className="space-y-5 border-l border-gray-100 pl-8">
                    <h3 className="text-xs font-black uppercase text-slate-400 tracking-wider">3. Media & Visuals</h3>
                    
                    {/* GALLERY */}
                    <div>
                        <label className="input-label">Gallery Images</label>
                        <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl mb-4">
                            ...
                            <label className="w-full py-2 border-2 border-dashed border-slate-300 rounded-lg flex items-center justify-center cursor-pointer hover:bg-white hover:border-slate-400 transition-all text-slate-500 gap-2">
                                <input type="file" multiple accept="image/*" className="hidden" onChange={handleMultiImageUpload} />
                                {uploading ? <Loader2 className="animate-spin" size={14} /> : <UploadCloud size={14} />} <span className="text-[10px] font-bold uppercase">{uploading ? '...' : 'Add Images'}</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="input-label">Before Image</label>
                            <div className="relative w-full aspect-square bg-slate-100 rounded-lg border border-slate-200 overflow-hidden group">
                                {currentProduct.before_image ? (
                                    <>
                                    <img src={currentProduct.before_image} className="w-full h-full object-cover" />
                                    <button type="button" onClick={() => setCurrentProduct({...currentProduct, before_image: null})} className="absolute top-1 right-1 bg-black/50 text-white p-1 rounded-full"><X size={12}/></button>
                                    </>
                                ) : (
                                    <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer text-slate-400 hover:text-black hover:bg-slate-200 transition-colors">
                                        <input type="file" accept="image/*" className="hidden" onChange={(e) => handleSingleUpload(e, 'before_image')} />
                                        <UploadCloud size={20} /> <span className="text-[9px] font-bold mt-1">UPLOAD</span>
                                    </label>
                                )}
                            </div>
                        </div>
                        <div>
                            <label className="input-label">After Image</label>
                            <div className="relative w-full aspect-square bg-slate-100 rounded-lg border border-slate-200 overflow-hidden group">
                                {currentProduct.after_image ? (
                                    <>
                                    <img src={currentProduct.after_image} className="w-full h-full object-cover" />
                                    <button type="button" onClick={() => setCurrentProduct({...currentProduct, after_image: null})} className="absolute top-1 right-1 bg-black/50 text-white p-1 rounded-full"><X size={12}/></button>
                                    </>
                                ) : (
                                    <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer text-slate-400 hover:text-black hover:bg-slate-200 transition-colors">
                                        <input type="file" accept="image/*" className="hidden" onChange={(e) => handleSingleUpload(e, 'after_image')} />
                                        <UploadCloud size={20} /> <span className="text-[9px] font-bold mt-1">UPLOAD</span>
                                    </label>
                                )}
                            </div>
                    ...
                    <div><label className="input-label">Short Description</label><textarea rows={2} className="input-field resize-none" placeholder="Brief summary..." value={currentProduct.short_description} onChange={e => setCurrentProduct({...currentProduct, short_description: e.target.value})} /></div>
                    
                    {/* IMPROVED MAIN DESCRIPTION */}
                    <div>
                        <label className="input-label flex justify-between">
                            <span>Main Description</span>
                            <span className="text-[9px] text-green-600 normal-case font-medium">Tip: Press Enter for new paragraphs</span>
                        </label>
                        <textarea 
                            rows={8} 
