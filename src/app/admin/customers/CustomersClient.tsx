              ...
              <tbody className="divide-y divide-slate-100">
                {filteredUsers.length > 0 ? filteredUsers.map(user => (
                  <tr key={user.id} className={`hover:bg-slate-50 transition-colors ${user.is_blocked ? 'bg-red-50/50' : ''}`}>
                    ...
                    <td className="px-6 py-4">
                      {user.is_blocked ? (
                        <span className="px-2 py-1 rounded-full text-[10px] font-black uppercase bg-red-100 text-red-700 border border-red-200">Blocked</span>
                      ) : (
                        <span className="px-2 py-1 rounded-full text-[10px] font-black uppercase bg-emerald-100 text-emerald-700 border border-emerald-200">Active</span>
                      )}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        <button
                          ...
                          className={`p-2 rounded transition-colors border ${user.is_blocked ? 'text-emerald-600 border-emerald-200 hover:bg-emerald-50' : 'text-amber-500 border-amber-200 hover:bg-amber-50'}`}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <h3 className="text-xs font-black uppercase text-slate-400 tracking-wider">Basic Info</h3>
                  <div>
                ...
                <div className="space-y-4">
                  <h3 className="text-xs font-black uppercase text-slate-400 tracking-wider">Default Address</h3>
                    ...
                    {getPrimaryAddress(selectedUser) ? (
                      <>
                        ...
                        <div className="text-sm font-medium text-slate-700 leading-relaxed ml-6">
                          {getPrimaryAddress(selectedUser)?.house_no && (
                            <div>{getPrimaryAddress(selectedUser)?.house_no},</div>
                          )}
                          <div>{getPrimaryAddress(selectedUser)?.street}</div>
                          <div>{getPrimaryAddress(selectedUser)?.city}, {getPrimaryAddress(selectedUser)?.state}</div>
                          <div className="font-bold text-slate-900">{getPrimaryAddress(selectedUser)?.pincode}</div>
                        </div>
                      </>
                    ) : (
                ...
                <div className="flex gap-4">
                  <button
                    ...
                    className={`flex-1 py-3 border-2 rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-all ${
                      selectedUser.is_blocked ? 'border-emerald-100 text-emerald-600 hover:bg-emerald-50' : 'border-amber-100 text-amber-600 hover:bg-amber-50'
                    }`}
                  >
