...
import {
  Search, ChevronDown, Download, Phone, Loader2, Eye,
  MapPin, CreditCard, Calendar, X, ImageOff, ChevronLeft, ChevronRight
} from 'lucide-react';
...
export default function OrdersClient({ initialOrders }: Props) {
  ...
  const handleExportCSV = () => {
    setExporting(true);
    try {
      const headers = ['Order ID', 'Customer Name', 'Phone', 'Address', 'Date', 'Total', 'Status', 'Payment Method'];
      const rows = filteredOrders.map(order => [
        order.id, getCustomerName(order), getCustomerPhone(order), getFullAddress(order),
        formatDate(order.created_at), order.total_amount, order.status, order.payment_method,
      ]);
  ...
  const getCustomerName = (o: Order) => {
    if (o.shipping_address && typeof o.shipping_address === 'object') {
      return `${o.shipping_address.firstName || ''} ${o.shipping_address.lastName || ''}`.trim() || 'Guest';
    }
  ...
  const getFullAddress = (o: Order) => {
    ...
    const { address, city, pincode } = o.shipping_address;
  ...
  const getStatusStyles = (s: OrderStatus) => {
    ...
    switch (s) {
      ...
      default: return `${base} bg-white border-slate-300 text-slate-900`;
    }
  };
  ...
  const filteredOrders = orders.filter((o) => {
    ...
    return (
      (o.id.toLowerCase().includes(q) || getCustomerName(o).toLowerCase().includes(q)) &&
  ...
  const currentOrders = filteredOrders.slice(indexOfFirst, indexOfFirst + itemsPerPage);
    ...
    <div className="min-h-screen pb-10 font-sans text-slate-900 bg-[#F8F9FC] p-6 overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <button
          ...
          className="w-fit bg-black text-white px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 hover:bg-slate-800 transition-colors shadow-sm disabled:opacity-50"
        >
          ...
          {['All', 'Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled'].map(s => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-bold transition-all border ${
                statusFilter === s ? 'bg-black text-white border-black' : 'bg-transparent text-slate-500 border-transparent hover:bg-slate-50 hover:text-black'
              }`}
            >
              {s}
            </button>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100">
                  {currentOrders.length > 0 ? currentOrders.map(order => (
                    <tr key={order.id} className={`group hover:bg-slate-50/50 transition-colors ${openDropdownId === order.id ? 'z-50' : 'z-0'} relative`}>
                      ...
                      <td className="px-6 py-4 relative status-dropdown-container whitespace-nowrap">
                        ...
                        {openDropdownId === order.id && (
                          <div className="absolute left-6 top-full mt-2 z-50 w-40 bg-white rounded-xl border border-slate-100 shadow-xl overflow-hidden">
                            <div className="py-1">
                              {['Pending', 'Processing', 'Shipped', 'Delivered', 'Cancelled'].map(s => (
                                ...
                                >
                                  {s}
                                  {order.status === s && <div className="w-1.5 h-1.5 rounded-full bg-black" />}
                                </button>
                              ))}
                            </div>
                          </div>
            ...
            {filteredOrders.length > 0 && (
              <div className="border-t border-slate-200 px-6 py-4 flex items-center justify-between bg-slate-50/50 rounded-b-2xl">
                <p className="text-xs font-medium text-slate-500">
                  Showing <span className="font-bold text-slate-900">{indexOfFirst + 1}</span> to{' '}
                  <span className="font-bold text-slate-900">{Math.min(indexOfFirst + itemsPerPage, filteredOrders.length)}</span> of{' '}
            ...
            <div className="flex justify-between items-center px-8 py-6 border-b border-slate-100 bg-slate-50">
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium flex items-center gap-1.5">
                  <Calendar size={12} /> {formatDate(selectedOrder.created_at)}
                </p>
              </div>
