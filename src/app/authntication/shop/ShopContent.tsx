...
export default function ShopContent({ initialProducts, initialWishlist, userId }: ShopContentProps) {
  ...
  const addToCart = async (e: React.MouseEvent, product: Product) => {
    ...
    try {
      const { data: existingItem } = await supabase.from('cart_items').select('id, quantity').eq('user_id', userId).eq('product_id', product.id).single();
      if (existingItem) await supabase.from('cart_items').update({ quantity: existingItem.quantity + 1 }).eq('id', existingItem.id);
      else await supabase.from('cart_items').insert([{ user_id: userId, product_id: product.id, quantity: 1 }]);
  ...
  const toggleWishlist = async (e: React.MouseEvent, product: Product) => {
    ...
    if (isLiked) await supabase.from('wishlist_items').delete().eq('user_id', userId).eq('product_id', product.id);
    else {
                      ...
                      >
                        {validImage ? (
                          <Image 
                             ...
                             alt={product.name} 
                             fill
                             // OPTIMIZATION: Small file for mobile, medium for tablet, large for desktop
