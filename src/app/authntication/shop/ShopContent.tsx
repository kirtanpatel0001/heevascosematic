...
// Map URL slugs (from ShopByConcernFinal) → CONCERNS labels
...
const Toast = ({
  ...
  const validImage = product.image_url || product.images?.[0] || null;
  ...
  return (
    <div className="fixed top-24 right-6 z-[100] animate-in slide-in-from-top-5 fade-in duration-300">
      <div className="bg-white border border-gray-100 shadow-2xl rounded-lg p-4 flex gap-4 items-center max-w-sm w-full pr-10 relative">
        ...
        <div className="w-12 h-16 bg-gray-100 rounded-md overflow-hidden flex-shrink-0 border border-gray-200 relative">
          {validImage ? (
            <Image src={validImage} alt={product.name} fill sizes="48px" className="object-cover" />
          ) : (
        ...
        <div>
          <h4 className={`text-xs font-bold uppercase tracking-wider mb-1 flex items-center gap-1 ${type === 'cart' ? 'text-green-600' : 'text-red-500'}`}>
            {type === 'cart' ? <><CheckCircle size={14} /> Added to Bag</> : <><Heart size={14} fill="currentColor" /> Added to Wishlist</>}
          </h4>
...
const FilterSection = ({
  ...
  return (
    <div className="border-b border-gray-100 py-6 last:border-0">
      <button onClick={toggle} className="flex items-center justify-between w-full group mb-4">
...
const FilterPanel = React.memo(({
  ...
  return (
    <div className="space-y-2 pr-2">
        ...
        <div className="space-y-3">
          {CATEGORIES.map((cat) => (
            <label key={cat} className="flex items-center gap-3 cursor-pointer group">
              ...
              <input type="checkbox" className="hidden" checked={selectedCategories.includes(cat)} onChange={() => toggleSelection(cat, selectedCategories, setSelectedCategories)} />
        ...
        <div className="space-y-3">
          {CONCERNS.map((concern) => (
            <label key={concern} className="flex items-center gap-3 cursor-pointer group">
              ...
              <input type="checkbox" className="hidden" checked={selectedConcerns.includes(concern)} onChange={() => toggleSelection(concern, selectedConcerns, setSelectedConcerns)} />
        ...
        <div className="space-y-3">
          {HAIR_TYPES.map((type) => (
            <label key={type} className="flex items-center gap-3 cursor-pointer group">
              ...
              <input type="checkbox" className="hidden" checked={selectedHairTypes.includes(type)} onChange={() => toggleSelection(type, selectedHairTypes, setSelectedHairTypes)} />
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              onChange={(e) => setPriceRange((r) => ({ ...r, min: e.target.value ? Number(e.target.value) : '' }))}
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              onChange={(e) => setPriceRange((r) => ({ ...r, max: e.target.value ? Number(e.target.value) : '' }))}
              className="w-full bg-gray-50 border border-gray-200 p-2 pl-6 text-xs rounded-sm focus:outline-none focus:border-black"
            />
          </div>
        </div>
      ...
      {/* CLEAR */}
      <button onClick={clearFilters} className="w-full mt-8 py-3 text-xs font-bold uppercase tracking-widest border border-black hover:bg-black hover:text-white transition-all">
        Reset Filters
      </button>
    </div>
  );
...
const ProductCard = React.memo(({
  product, isWishlisted, isLoading, onAddToCart, onToggleWishlist,
}: ProductCardProps) => {
  ...
  return (
    <div className="group relative mx-auto w-full">
      ...
      >
        ...
        {product.stock <= 0 && (
          <span className="absolute top-4 left-4 bg-white/90 backdrop-blur text-[9px] font-bold uppercase tracking-[0.2em] px-3 py-1.5 text-gray-500 z-10">
            Sold Out
          </span>
        )}
        ...
        <div className="absolute inset-x-0 bottom-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out z-10">
          <button
            ...
            className="w-full bg-white text-black text-[10px] font-bold uppercase tracking-[0.2em] py-3.5 hover:bg-black hover:text-white transition-colors shadow-xl flex items-center justify-center gap-2"
          >
            {isLoading ? <Loader2 className="animate-spin" size={12} /> : <>Add to Cart <ShoppingBag size={12} /></>}
          </button>
        </div>
        ...
        <button
          ...
          className={`absolute top-4 right-4 transition-all duration-300 z-10 p-2 rounded-full ${isWishlisted ? 'text-red-500 opacity-100 scale-110' : 'text-gray-400 opacity-0 group-hover:opacity-100 hover:text-black hover:scale-110'}`}
        >
      ...
      <div className="text-left space-y-1.5 px-1">
        ...
        <Link href={`/product/${product.id}`}>
          <h3 className="text-sm font-medium text-gray-900 tracking-wide cursor-pointer line-clamp-1 group-hover:underline decoration-1 underline-offset-4">
            {product.name}
          </h3>
        </Link>
...
export default function ShopContent({ initialProducts, initialWishlist, userId }: ShopContentProps) {
  const searchParams = useSearchParams();
  const [products] = useState<Product[]>(initialProducts);
  const [wishlistIds, setWishlistIds] = useState<Set<string>>(new Set(initialWishlist));

  // ── Read ?concern= from URL and map to filter label on first render ──
  const getInitialConcerns = (): string[] => {
    const slug = searchParams.get('concern');
    if (!slug) return [];
    const mapped = CONCERN_SLUG_MAP[slug];
  ...
  // Filters — selectedConcerns initialised from URL param
  const [searchTerm,          setSearchTerm]          = useState('');
  const [selectedCategories,  setSelectedCategories]  = useState<string[]>([]);
  const [selectedConcerns,    setSelectedConcerns]    = useState<string[]>(getInitialConcerns);
  ...
  // ── If the URL param changes (e.g. browser back/forward), sync the filter ──
  useEffect(() => {
    const slug = searchParams.get('concern');
    if (!slug) return;
    const mapped = CONCERN_SLUG_MAP[slug];
    if (mapped) setSelectedConcerns([mapped]);
  }, [searchParams]);

  // ── Auto-open mobile filter drawer when a concern param is present ──
  useEffect(() => {
    if (searchParams.get('concern') && window.innerWidth < 768) {
  ...
  const showToast = useCallback((product: Product, type: 'cart' | 'wishlist') => {
    if (toastTimerRef.current) clearTimeout(toastTimerRef.current);
    setToast({ show: true, product, type });
  ...
  const filteredProducts = useMemo(() => {
    const minPrice = typeof priceRange.min === 'number' ? priceRange.min : 0;
    const maxPrice = typeof priceRange.max === 'number' ? priceRange.max : 1_000_000;
    const search   = searchTerm.toLowerCase();

    const result = products.filter((p) =>
      p.name.toLowerCase().includes(search) &&
      (selectedCategories.length === 0 || selectedCategories.includes(p.category)) &&
      (selectedConcerns.length  === 0 || p.hair_concern?.some((c) => selectedConcerns.includes(c))) &&
      (selectedHairTypes.length === 0 || p.hair_type?.some((t) => selectedHairTypes.includes(t))) &&
      p.price >= minPrice && p.price <= maxPrice &&
      (!inStockOnly || p.stock > 0)
    );

    if (sortOrder === 'low-to-high') return [...result].sort((a, b) => a.price - b.price);
    if (sortOrder === 'high-to-low') return [...result].sort((a, b) => b.price - a.price);
    return result;
  }, [searchTerm, selectedCategories, selectedConcerns, selectedHairTypes, priceRange, inStockOnly, sortOrder, products]);

  // ── CART ──────────────────────────────────
  const addToCart = useCallback(async (e: React.MouseEvent, product: Product) => {
    e.preventDefault(); e.stopPropagation();
    if (!userId) { setIsLoginModalOpen(true); return; }
    ...
    try {
      ...
      if (existing) {
        ...
        await supabase.from('cart_items').insert([{ user_id: userId, product_id: product.id, quantity: 1 }]);
      }
      showToast(product, 'cart');
    } catch (err) {
      console.error(err);
    } finally {
  ...
  // ── WISHLIST ──────────────────────────────
  const toggleWishlist = useCallback(async (e: React.MouseEvent, product: Product) => {
    e.preventDefault(); e.stopPropagation();
    if (!userId) { setIsLoginModalOpen(true); return; }
    ...
    setWishlistIds((prev) => {
      ...
      isLiked ? next.delete(product.id) : next.add(product.id);
      return next;
    });
    ...
    if (isLiked) {
      ...
      const { error } = await supabase.from('wishlist_items').insert([{ user_id: userId, product_id: product.id }]);
      if (!error) showToast(product, 'wishlist');
      if (error) setWishlistIds((prev) => { const next = new Set(prev); next.delete(product.id); return next; });
    }
  }, [userId, wishlistIds, showToast]);

  return (
    <div className="min-h-screen bg-white font-sans text-gray-900 relative overflow-x-hidden">
      <Toast show={toast.show} onClose={closeToast} product={toast.product} type={toast.type} />
      ...
      <div className="bg-white border-b border-gray-100 py-16">
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          {/* Show active concern filter as a visible badge */}
          {selectedConcerns.length > 0 && (
            <div className="mt-4 flex items-center justify-center gap-2 flex-wrap">
              {selectedConcerns.map((c) => (
                <span key={c} className="inline-flex items-center gap-1.5 px-3 py-1 bg-black text-white text-[10px] font-bold uppercase tracking-widest rounded-full">
                  {c}
                  <button onClick={() => setSelectedConcerns((prev) => prev.filter((x) => x !== c))} className="hover:opacity-70">
                    <X size={10} />
                  </button>
                </span>
              ))}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-12">
        ...
        <div className="md:hidden flex justify-between items-center mb-8 sticky top-0 bg-white z-20 py-4 border-b">
          <button
            ...
            className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest border border-gray-200 px-4 py-3 hover:bg-black hover:text-white transition-colors"
          >
          ...
          <div className="flex-1">
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center py-32 bg-gray-50 rounded-lg">
                ...
                <button onClick={clearFilters} className="text-[10px] font-bold uppercase border-b border-black pb-1 hover:opacity-50 tracking-widest">
                  Clear All Filters
                </button>
              </div>
            ) : (
      ...
      {isLoginModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
          ...
          <div className="bg-white p-10 max-w-sm w-full relative z-10 text-center shadow-2xl rounded-sm">
            ...
            <div className="space-y-3">
              <Link href="/login" className="block w-full bg-black text-white text-[10px] font-bold uppercase tracking-[0.2em] py-3.5 hover:bg-gray-800 transition-colors">
                Sign In
              </Link>
              <button onClick={() => setIsLoginModalOpen(false)} className="block w-full bg-transparent text-gray-400 text-[10px] font-bold uppercase tracking-[0.2em] py-3.5 hover:text-black transition-colors">
                Continue Shopping
              </button>
            </div>
          </div>
