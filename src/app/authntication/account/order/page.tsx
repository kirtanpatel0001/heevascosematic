...
import {
  ...
  Star, Upload, MessageSquarePlus, ClipboardCheck, RotateCw
} from 'lucide-react';
...
/* ---------------- ORDER TIMELINE ---------------- */
const ORDER_STAGES = [
  { key: 'pending',    label: 'Accepted',   icon: ClipboardCheck, description: 'Order confirmed' },
  { key: 'processing', label: 'Processing', icon: RotateCw,        description: 'Being packed'    },
  { key: 'shipped',    label: 'Shipped',    icon: Truck,           description: 'On the way'      },
  { key: 'delivered',  label: 'Delivered',  icon: CheckCircle2,    description: 'Delivered'       },
];
...
const OrderTimeline = ({ status }: { status: string }) => {
  ...
  if (isCancelled) {
    return (
      <div className="px-6 py-5 border-b border-gray-100 bg-red-50/60">
        <p className="text-xs font-black uppercase text-slate-400 tracking-wider mb-3">Order Status</p>
  ...
  return (
    <div className="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-white">
      <p className="text-xs font-black uppercase text-slate-400 tracking-wider mb-4">Order Timeline</p>
      <div className="relative flex items-start justify-between">
        <div className="absolute top-[18px] left-[18px] right-[18px] h-0.5 bg-gray-200 z-0" />
        <div
          className="absolute top-[18px] left-[18px] h-0.5 bg-gradient-to-r from-emerald-400 to-emerald-500 z-0 transition-all duration-700"
          style={{
            width: activeIndex <= 0
              ? '0%'
              : activeIndex >= ORDER_STAGES.length - 1
              ? 'calc(100% - 36px)'
              : `calc(${(activeIndex / (ORDER_STAGES.length - 1)) * 100}% - ${(activeIndex / (ORDER_STAGES.length - 1)) * 36}px)`,
          }}
        />
          ...
          return (
            <div key={stage.key} className="relative flex flex-col items-center z-10 flex-1">
              <div className={`w-9 h-9 rounded-full flex items-center justify-center border-2 transition-all duration-500
                ...
                <Icon size={15} className={isCompleted ? 'text-white' : 'text-gray-300'} strokeWidth={isActive ? 2.5 : 2} />
                {isActive && <span className="absolute inset-0 rounded-full bg-emerald-400 animate-ping opacity-30" />}
              </div>
              <div className="mt-2 text-center px-1">
                <p className={`text-[10px] font-black uppercase tracking-wide leading-tight
                  ${isCompleted ? isActive ? 'text-emerald-600' : 'text-slate-700' : 'text-slate-300'}`}>
                  {stage.label}
                </p>
                <p className={`text-[9px] leading-tight mt-0.5 hidden sm:block
                  ${isCompleted ? 'text-slate-400' : 'text-slate-200'}`}>
                  {stage.description}
                </p>
              </div>
            </div>
...
const ReviewModal = ({ isOpen, onClose, product, orderId, onSuccess }: {
  isOpen: boolean; onClose: () => void; product: any; orderId: string; onSuccess: () => void;
}) => {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    if (res?.error) { toast.error('Submission Failed', { description: res.error }); }
    else { toast.success('Review Posted', { description: 'Thank you! Your feedback has been published.' }); onSuccess(); onClose(); }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
        <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          ...
          <button onClick={onClose} className="p-1 hover:bg-gray-200 rounded-full"><X size={20} className="text-slate-400" /></button>
        </div>
        <div className="p-6 overflow-y-auto">
          <div className="flex gap-4 mb-6 bg-gray-50 p-3 rounded-xl border border-gray-100">
            <div className="relative w-14 h-14 bg-white rounded-lg overflow-hidden border border-gray-200 shadow-sm">
              {product.image_url && <Image src={product.image_url} alt="product" fill className="object-cover" unoptimized />}
            </div>
          ...
          <form action={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <textarea name="comment" required placeholder="How was the quality? Did it meet your expectations?"
                ...
                <div className="flex gap-2">
                  {previews.map((src, i) => (
                    <div key={i} className="w-16 h-16 relative rounded-lg border border-gray-200 overflow-hidden group">
                      ...
                      <button type="button" onClick={() => removeImage(i)} className="absolute top-0 right-0 p-1 bg-black/50 text-white hover:bg-red-500 opacity-0 group-hover:opacity-100"><X size={10} /></button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <button disabled={isSubmitting} type="submit"
              className="w-full bg-black text-white py-3.5 rounded-xl font-bold text-sm hover:bg-zinc-800 disabled:opacity-50 transition-all flex items-center justify-center gap-2">
...
export default function OrdersPage() {
  ...
  const fetchData = async () => {
    ...
    const { data: ordersData, error } = await supabase
      .from('orders')
      .select(`*, items:order_items (product_id, quantity, price, product:products (name, category, image_url))`)
      .eq('user_id', user.id)
      .order('created_at', { ascending: false });

    const { data: reviewsData } = await supabase.from('reviews').select('product_id').eq('user_id', user.id);
    if (reviewsData) setReviewedProductIds(new Set<string>(reviewsData.map((r: any) => r.product_id)));
    if (!error && ordersData) setOrders(ordersData as any);
    setLoading(false);
  };
        ...
        <div className="space-y-4">
          {orders.length === 0 ? (
            ...
            orders.map((order) => {
              ...
              return (
                <div key={order.id} onClick={() => { setSelectedOrder(order); setIsModalOpen(true); }}
                  className="bg-white rounded-2xl border border-gray-200 p-5 cursor-pointer hover:shadow-lg transition-all duration-200 group">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-medium text-slate-400">{new Date(order.created_at).toLocaleDateString()}</span>
                    </div>
                  ...
                  <div className="flex items-start gap-4">
                    <div className="relative w-20 h-20 bg-gray-50 rounded-lg overflow-hidden border border-gray-100 flex-shrink-0">
                      {firstItem?.product.image_url
                        ? <Image src={firstItem.product.image_url} alt="product" fill className="object-cover" unoptimized />
                        : <ShoppingBag className="m-auto text-slate-300" size={24} />}
                      {otherItemsCount > 0 && (
                        <div className="absolute inset-0 bg-black/60 flex items-center justify-center text-white text-xs font-bold">+{otherItemsCount}</div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        {order.status === 'Delivered' && (
                          <button onClick={(e) => { e.stopPropagation(); if (!isReviewed && firstItem) openReviewModal(firstItem.product, firstItem.product_id, order.id); }}
                            disabled={!!isReviewed}
                            className={`flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded-lg transition-all shadow-sm
                              ${isReviewed ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-zinc-900 text-white hover:bg-zinc-700 active:scale-95'}`}>
                            {isReviewed ? <><CheckCircle2 size={12} /> Reviewed</> : <><MessageSquarePlus size={12} /> Review</>}
                          </button>
                        )}
                      </div>
                    </div>
          ...
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="flex-none px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-white">
              <div>
                ...
                <span className="font-mono text-xs font-bold bg-slate-50 border border-gray-200 px-1.5 py-0.5 rounded text-slate-600 mt-1 inline-block">
            ...
            <div className="flex-none"><OrderTimeline status={selectedOrder.status} /></div>
                    ...
                    <div key={idx} className="flex flex-col sm:flex-row gap-4 p-4 bg-white rounded-xl border border-gray-100 shadow-sm items-center">
                      <div className="flex items-center gap-4 w-full">
                        <div className="relative h-16 w-16 bg-gray-50 rounded-lg overflow-hidden border border-gray-200 flex-shrink-0">
                          {item.product.image_url && <Image src={item.product.image_url} alt="product" fill className="object-cover" unoptimized />}
                        </div>
                      ...
                      {selectedOrder.status === 'Delivered' && (
                        <div className="w-full sm:w-auto flex-shrink-0">
                          <button onClick={(e) => { e.stopPropagation(); if (!isItemReviewed) openReviewModal(item.product, item.product_id, selectedOrder.id); }}
                            disabled={!!isItemReviewed}
                            className={`w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2.5 text-[11px] font-bold uppercase tracking-wider rounded-lg transition-all whitespace-nowrap
                              ${isItemReviewed ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-zinc-900 text-white hover:bg-zinc-700 active:scale-95'}`}>
                            {isItemReviewed ? <><CheckCircle2 size={14} /> Reviewed</> : <><MessageSquarePlus size={14} /> Write Review</>}
                          </button>
                        </div>
                      )}
                    </div>
            ...
            <div className="flex-none p-5 border-t border-gray-100 bg-white flex flex-col gap-3">
              {isOrderCancellable(selectedOrder.status) && (
                <button onClick={() => handleCancelOrder(selectedOrder.id)} disabled={!!cancelling}
                  className="w-full py-3.5 bg-white border border-red-400 text-red-600 font-bold text-sm rounded-xl hover:bg-red-50 transition-all flex items-center justify-center gap-2">
                  {cancelling ? <Loader2 className="animate-spin" size={16} /> : <Ban size={16} />}
                  Cancel Order
                </button>
              )}

              {/* ✅ Direct PDF download — no preview, just download */}
              {isClient && settings && (
                <PDFDownloadLink
                  document={<InvoicePDF invoice={toInvoiceData(selectedOrder)} settings={settings} />}
                  fileName={`INV-${selectedOrder.id.slice(0, 8).toUpperCase()}.pdf`}
                  className="w-full"
                >
                  {({ loading: pdfLoading, error }) => (
                    <button
                      disabled={pdfLoading}
                      className="w-full bg-[#1a1a2e] text-white py-3.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 hover:bg-[#16213e] transition-all shadow-md disabled:opacity-60"
                    >
                      {pdfLoading
                        ? <><Loader2 className="animate-spin" size={16} /> Preparing Invoice...</>
