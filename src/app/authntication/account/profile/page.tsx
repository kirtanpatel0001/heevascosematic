...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // ✅ Fixed: correct auth redirect path
      if (!user) {
  ...
  const updatePassword = async () => {
    // ✅ Fixed: consistent 8 char minimum across all auth pages
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row font-sans text-gray-900">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="pt-4 flex justify-end">
                        <button
                          ...
                          disabled={saving}
                          className="bg-black text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-800 transition-colors disabled:opacity-70 flex items-center gap-2 shadow-sm"
                        >
                    ...
                    {!showAddressForm ? (
                          ...
                          <div key={addr.id} className="group border border-gray-200 rounded-xl p-5 hover:border-gray-300 transition-all bg-gray-50/50">
                            <div className="flex items-start justify-between">
                              ...
                              <div className="flex gap-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => handleEditAddress(addr)} className="p-2 text-gray-500 hover:text-black hover:bg-white rounded-lg transition-all">
                                  <Pencil size={16} />
                                </button>
                                <button onClick={() => deleteAddress(addr.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-white rounded-lg transition-all">
                                  <Trash2 size={16} />
                                </button>
                              </div>
                            </div>
                      ...
                      <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                        <div className="flex items-center justify-between mb-6">
                          ...
                          <button onClick={resetAddressForm} className="text-xs font-medium text-gray-500 hover:text-gray-900">
                            Cancel
                          </button>
                        </div>
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <div className="flex gap-2">
                              {['Home', 'Work', 'Other'].map(type => (
                                <button
                                  key={type}
                                  onClick={() => setAddressForm({ ...addressForm, labelType: type })}
                                  className={`px-4 py-2 rounded-full text-xs font-bold border transition-all ${
                                    addressForm.labelType === type
                            ...
                            {addressForm.labelType === 'Other' && (
                              <div className="mt-3">
                                <Input
                                  ...
                                  onChange={(e: any) => setAddressForm({ ...addressForm, customLabel: e.target.value })}
                                  placeholder="Ex: Mom's House"
                                />
                              </div>
                            )}
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <Input
                              ...
                              onChange={(e: any) => setAddressForm({ ...addressForm, houseNo: e.target.value })}
                            />
                            <Input
                              label="Street / Colony *"
                          ...
                          <Input
                            ...
                            maxLength={6}
                            onChange={(e: any) =>
                              setAddressForm({ ...addressForm, pincode: e.target.value.replace(/\D/g, '').slice(0, 6) })
                            }
                          />
                        </div>
                        ...
                        <div className="mt-6 flex gap-3 justify-end">
                          ...
                          <button
                            onClick={saveAddress}
                            disabled={saving}
                            className="bg-black text-white px-6 py-2 text-sm font-medium rounded-lg hover:bg-gray-800 transition-colors shadow-sm disabled:opacity-70 flex items-center gap-2"
                          >
                    ...
                    <div className="max-w-md space-y-5">
                      ...
                      <div className="pt-2">
                        <button
                          ...
                          disabled={saving}
                          className="bg-black text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-800 transition-colors flex items-center gap-2 shadow-sm disabled:opacity-70"
                        >
                          {saving && <Loader2 size={16} className="animate-spin" />}
                          Update Password
                        </button>
      ...
      {/* MOBILE BOTTOM NAV */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 z-50 px-6 py-3 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.03)]">
...
/* ---------------- HELPERS ---------------- */
function MobileBottomTab({ label, active, onClick, icon }: {
  label: string;
  active: boolean;
  onClick: () => void;
